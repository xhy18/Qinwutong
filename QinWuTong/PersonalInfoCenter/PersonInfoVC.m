...
- (void)viewDidLoad {
    ...
    CGFloat itemHei = 40;
    _itemName = [NSArray arrayWithObjects:@"用户名",@"姓名",@"QQ",@"微信",@"微博",@"邮箱",nil];
    _itemTitle = [NSArray arrayWithObjects:@"userName",@"name",@"qq",@"wechat",@"weibo",@"email",nil];
    ...
    if(btn.tag == 1){//切换为编辑页面
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:
                                 ...
                                 nil];
            [DIYAFNetworking PostHttpDataWithUrlStr:[NSString stringWithFormat:@"%@/app/client/appUser/completeIndividualInfo", baseUrl] isJson:true Dic:dic SuccessBlock:^(id responseObject){
...
-(void)getInfo{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [DIYAFNetworking GetHttpDataWithUrlStr:[NSString stringWithFormat:@"%@/app/client/appUser/personalInfo", baseUrl]Dic:dic SuccessBlock:^(id responseObject){
